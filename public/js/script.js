...
async function login(username, password) {
  try {
    const response = await fetch('/api/login.php', { // <-- updated endpoint
      method: 'POST',
...
async function register(username, name, email, password) {
  try {
    const response = await fetch('/api/register.php', { // <-- updated endpoint
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function initModals() {
  ...
  if (workoutForm) {
    workoutForm.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            const response = await fetch('/api/add_workout.php', {
                method: 'POST',
                headers: {
  ...
  if (addGoalForm) {
    addGoalForm.addEventListener('submit', async function(e) {
      ...
      if (result.success) {
        alert('Goal added!');
        await fetch('/api/update_goal_progress.php', {
...
async function initProgressCharts() {
  ...
  const token = localStorage.getItem('jwtToken');
  const metricRes = await fetch('/api/get_metric_trends.php', {
    headers: { 'Authorization': 'Bearer ' + token }
  });
  ...
  // Fetch workout stats
  const workoutRes = await fetch('/api/get_workout_stats.php', {
    headers: { 'Authorization': 'Bearer ' + token }
  });
...
async function fetchAndRenderWorkouts() {
    try {
        const token = localStorage.getItem('jwtToken');
        const response = await fetch('/api/get_workouts.php', {
            headers: {
                'Authorization': 'Bearer ' + token
            }
        });
...
async function fetchAndRenderTodayActivity() {
  const token = localStorage.getItem('jwtToken');
  const res = await fetch('/api/get_today_activity.php', {
    headers: { 'Authorization': 'Bearer ' + token }
  });
  const data = await res.json();
